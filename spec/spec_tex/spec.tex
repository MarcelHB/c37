...
void* ex_realloc(void* ptr, size_t size);
	...
	%----------------------------------------------------------------------------
	\newpage
	\subsubsection*{game\_messages.h}
	Da das Spiel etwas mehr als eine graphische Ausgabe benötigt, um sinnvoll mit dem Spieler zu interagieren, können verschiedene Vorgänge und Events dem Spieler mehr berichten.
		...
		\begin{lstlisting}[caption=message]{}
void message(const char* msg);
		\end{lstlisting}
		
	\begin{description}
		\item[Verwendung:] Fügt den String \textit{msg} an das vordere Ende des Nachrichtenstreams ein. Sind dadurch mehr Nachrichten enthalten als in \textit{MESSAGE\_STREAM\_LIMIT} zugelassen, werden Nachrichten am hinteren Ende gelöscht.
		\item[Parameter:] \hfill
		\begin{itemize}
			\item msg - Nachricht, die dem Spieler mitgeteilt werden soll.
		\end{itemize}
		\item[Rückgabe:] -
	\end{description}
	
		\begin{lstlisting}[caption=flush\_messages]{}
void flush_messages();
		\end{lstlisting}
		
	\begin{description}
		\item[Verwendung:] Leer den Nachrichtenstream.
		\item[Parameter:] -
		\item[Rückgabe:] -
	\end{description}
...
void spawn_uses_item(Spawn* spawn, Item* item);
	...
	Ein \textit{Spawn} ist die Struktur, die grundsätzlich alle Informationen enthält, die für spielbare und nichtspielbare Figuren benötigt wird.
...
Elemente der Struktur:
		\begin{itemize}
			...
			\item inventory - Ein Array, in dem Besitztümer liegen.
			\item inventory\_size - die Anzahl der Besitztümer
		\end{itemize}
		
		\begin{lstlisting}[caption=spawn\_spawn\_collision]{}
void spawn_spawn_collision(Spawn* spawn_a, Spawn* spawn_b);
		\end{lstlisting}
...
Die \textit{Tile}-Struktur beschreibt ein Spielfeld, das aufgrund seiner Beschaffenheit für die Spiellogik wichtig ist. Die Art und der Zustand einer Kachel bestimmt, wie diese in der Ausgabe erscheinen wird.
...
Elemente der Struktur:
		\begin{itemize}
			...
			\item properties - eine von \textit{type} abhängig geladene Struktur für zusätzliche Eigenschaften
			\item items - Ein Array, in dem hier abgelegte Items liegen.
...
void spawn_tile_collision(Spawn* spawn, Tile* tile);
		\end{lstlisting}
		
	\begin{description}
		\item[Verwendung:] Es wird die Aktion ausgeführt, die eintritt, wenn \textit{spawn} die Kachel \texit{it} betreten möchte. Beispielsweise, ob er diese überhaupt betreten darf.
		\item[Parameter:] \hfill
		\begin{itemize}
...
Elemente der Struktur:
		\begin{itemize}
			...
			\item spawns - Array, das die Spawns enthält.
...
Spawn* get_spawn_at(unsigned int x, unsigned int y);
		\end{lstlisting}
...
int load_map(char* name);
	...
	\begin{description}
		...
		\end{itemize}
		\item[Rückgabe:] Rückgabe eines Status, 1 bedeutet Erfolg, 0 einen Fehler.
	\end{description}
...
void flush_map();
	...
	\begin{description}
		\item[Verwendung:] Löscht die Karte und alle verwendeten Komponenten aus dem Speicher.
		\item[Parameter:] -
		\item[Rückgabe:] -
	\end{description}
	...
	Diese Header-Datei enthält Funktionen, die dem Programmfluss des Programms und der Übersetzung von Karte zu Ausgabe dienlich sind.
		...
		\begin{lstlisting}[caption=pause\_game]{}
void pause_game();
		\end{lstlisting}
		
	\begin{description}
		\item[Verwendung:] Hält das Spielgesehen an.
		\item[Parameter:] -
		\item[Rückgabe:] -
	\end{description}
	
		\begin{lstlisting}[caption=continue\_game]{}
void continue_game();
		\end{lstlisting}
		
	\begin{description}
		\item[Verwendung:] Setzt das Spielgesehen fort.
		\item[Parameter:] -
		\item[Rückgabe:] -
	\end{description}
